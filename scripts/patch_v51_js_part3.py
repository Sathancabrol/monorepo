# -*- coding: utf-8 -*-
"""
Implement Enrobes 2D Simulation and update setSchemaViewMode in section_js_part3.py
"""

enrobes_2d_js_code = r'''
    // ==========================================
    ...
    function setSchemaViewMode(mode) {
        document.querySelectorAll('.schema-view-btn').forEach(b => b.classList.remove('active'));
        document.getElementById('btn-schema-' + mode.replace('_', ''))?.classList.add('active');

        const tView = document.getElementById('tech-tasksheets-view');
...
# Replace setSchemaViewMode in section_js_part3.py
pos_schema = text.find('function setSchemaViewMode(mode) {')
pos_toggle_comp = text.find('function toggleCompactageAnimation() {')

if pos_schema != -1 and pos_toggle_comp != -1:
    text = text[:pos_schema] + enrobes_2d_js_code.strip() + "\n\n    " + text[pos_toggle_comp:]
    print("Enrobes 2D Simulation code and setSchemaViewMode updated successfully in section_js_part3.py!")
else:
    print("Error locating setSchemaViewMode in section_js_part3.py")
